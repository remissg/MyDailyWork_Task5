...
import { Plus, MoreVertical, Calendar, Clock, Loader2 } from 'lucide-react';
...
const StatCard = ({ label, value, trend, trendUp, to }) => {
    const CardContent = (
        <motion.div
            whileHover={{ y: -5 }}
            className={`bg-white p-6 rounded-2xl border border-slate-100 shadow-sm hover:shadow-md transition-all ${to ? 'cursor-pointer group' : ''}`}
        >
            <p className="text-slate-500 text-sm mb-2">{label}</p>
            <div className="flex items-end justify-between">
                <h3 className={`text-3xl font-bold text-slate-800 ${to ? 'group-hover:text-primary transition-colors' : ''}`}>{value}</h3>
...
const ProjectCard = ({ project }) => {
    ...
    return (
        <motion.div
            ...
            className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm transition-all group cursor-pointer hover:shadow-md"
        >
            ...
            <div className="flex items-center justify-between mt-auto">
                <div className="flex -space-x-2">
                    {project.members && project.members.map((m, i) => (
                        <div key={i} className="w-8 h-8 rounded-full bg-white border-2 border-white flex items-center justify-center text-xs font-medium text-slate-600 ring-2 ring-transparent group-hover:ring-primary/20 transition-all shadow-sm">
                            {m}
                        </div>
                    ))}
                </div>
        ...
        <div className="space-y-8">
            <div className="flex items-center justify-between">
                <div>
                    ...
                    <p className="text-slate-500">Welcome back! Manage your projects efficiently.</p>
                </div>
                <button
            ...
            <div>
                ...
                {projects.length === 0 ? (
                    <div className="text-center py-20 bg-white rounded-2xl border border-slate-200 border-dashed">
                        <p className="text-slate-500 text-lg mb-4">No projects yet. Create one to get started!</p>
                        <button
                            onClick={() => setIsModalOpen(true)}
